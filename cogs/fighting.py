...
class Fighting(commands.Cog):
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            create_option(
                name="user", description="User to fight", option_type=6, required=True
            ),
        ],
    ...
    async def fight(self, ctx, enemy: discord.Member):
        ...
        if (
            enemy.status != discord.Status.online
            and ctx.author.id != bot_enums.OWNER_ID.value
        ):
            await ctx.send("They do not want to fight you.")
            return
        ...
        starter_attack = self.base_attack + (
            starter_user.level * self.attack_scale_factor
        )
        starter_defense = self.base_defense + (
            starter_user.level * self.defense_scale_factor
        )

        enemy_attack = self.base_attack + (enemy_user.level * self.attack_scale_factor)
        enemy_defense = self.base_defense + (
            enemy_user.level * self.defense_scale_factor
        )

        damage_to_enemy = round(
            starter_attack * (min(starter_attack / enemy_defense, 1)), 4
        )
        damage_to_starter = round(
            enemy_attack * (min(enemy_attack / starter_defense, 1)), 4
        )
        ...
        while True:
            ...
            await ctx.send(
                "",
                embed=svc.Games.create_fight_view(
                    starter_health, enemy_health, ctx.author, enemy
                ),
            )
            ...
            if chance_to_damage_starter <= self.base_hit_chance:
                damage_variation = damage_to_starter + random.randint(
                    0, self.max_damage_variation
                )
                ...
                if chance_to_crit <= self.base_crit_chance:
                    ...
                    await ctx.send(
                        f"{enemy.mention} hit {ctx.author.mention} for {damage_variation}!"
                    )
            ...
            if chance_to_damage_enemy <= self.base_hit_chance:
                damage_variation = damage_to_enemy + random.randint(
                    0, self.max_damage_variation
                )
                ...
                if chance_to_crit <= self.base_crit_chance:
                    ...
                    await ctx.send(
                        f"{ctx.author.mention} hit {enemy.mention} for {damage_variation}!"
                    )
        ...
        exp_reward = int(
            (100 + random.randint(0, self.max_reward_variation))
            * (
                (loser_user.level / winner_user.level)
                * random.uniform(1, self.max_level_impact_variation)
                * 0.75
            )
        )
        vbuck_reward = int(
            (1000 + random.randint(0, self.max_reward_variation))
            * (
                (loser_user.level / winner_user.level)
                * random.uniform(1, self.max_level_impact_variation)
            )
        ...
        await ctx.send(
            f"{winner.mention} has won! They gained {exp_reward} EXP and {vbuck_reward} V-Bucks!"
        )
        ...
        await ctx.send(
            f"{loser.mention} has lost and does not have insurance! They lose {vbuck_loss} in medical bills."
        )
