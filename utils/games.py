...
card_suits = {1: "Hearts", 2: "Spades", 3: "Clubs", 4: "Diamonds"}
...
def create_card_view(member, cards: list, color: discord.Color):
    ...
    for card in cards:
        card_embed.add_field(
            name=card_names.get(card[0]), value=card_suits.get(card[1])
        )
...
def create_fight_view(
    ...
    fight_embed = discord.Embed(
        title=title, description=description, color=discord.Color.random_color()
    )

    fight_embed.set_thumbnail(url=url)

    fight_embed.add_field(
        name=f"{starter_name}", value=f"Health: {int(starter_health)}"
    )
