...
class Admin(commands.Cog):
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            create_option(
                name="user", description="User to update", option_type=6, required=True
            ),
            create_option(
                name="money",
    ...
    async def update_vbucks(self, ctx, member: discord.Member, money: int):
        ...
        await messaging.send_message_embed(
            ctx, "", f"{member.mention}'s V-Buck amount has been updated to **{money}**"
        )
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            ...
            create_option(
                name="xp", description="New XP Amount", option_type=4, required=True
            ),
        ],
        guild_ids=level.get_guild_ids(),
    )
    ...
    async def update_exp(self, ctx, member: discord.Member, exp: int):
        ...
        if check is None:
            await messaging.send_message_embed(
                ctx, "Update EXP", f"{member.mention}'s XP has been set to {exp}"
            )
        else:
            await messaging.send_message_embed(
                ctx,
                "Update EXP",
                f"{member.mention}'s XP has been set to **{exp}** and their level has changed to **{check}**",
            )
    ...
    @cog_ext.cog_slash(
        ...
        options=[
            ...
            create_option(
                name="level", description="New User Level", option_type=4, required=True
            ),
        ],
        guild_ids=level.get_guild_ids(),
    )
    ...
    async def set_user_level(self, ctx, member: discord.Member, level: int):
        ...
        await messaging.send_message_embed(
            ctx,
            message=f"{member.mention}'s level is now **{level}**. XP is **{required_exp}**",
        )
    ...
    async def create_account(self, ctx, member: discord.Member):
        ...
        await messaging.send_message_embed(
            ctx, message=f"{member.display_name}'s account was created."
        )
    ...
    async def talk_ch(
        self, ctx: SlashContext, message, channel: discord.abc.GuildChannel
    ):
    ...
    async def change_currency_name(self, ctx: SlashContext, c_name):
        ...
        await messaging.send_message_embed(
            ctx, code_block=f"Name has changed to: {c_name}"
        )
